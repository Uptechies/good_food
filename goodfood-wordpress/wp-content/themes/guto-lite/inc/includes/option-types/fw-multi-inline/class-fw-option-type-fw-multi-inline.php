...
class FW_Option_Type_FwMultiInline extends FW_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		return fw_render_view( GUTO_LITE_INC . '/includes/option-types/fw-multi-inline/view.php', array(
			'id'	 => $id,
			'option' => $option,
			'data'	 => $data
		) );
	...
	protected function _get_defaults() {
		...
		return array(
			...
			'fw_multi_options'	 => array(
				'firstoption' => array(
					'type'		 => 'select',
					'title'		 => esc_html__( 'Title', 'guto-lite' ),
					'choices'	 => array(
						'select1'	 => esc_html__( 'select1', 'guto-lite' ),
						'select2'	 => esc_html__( 'select2', 'guto-lite' ),
						'select3'	 => esc_html__( 'select3', 'guto-lite' )
					),
				)
			),
			'groupname'			 => ''
